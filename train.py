...
dataloader = DataLoader(FlyingChairsDataset("/home/moriyama/FlyingChairs2/"), batch_size=batch_size)
# dataloader = ("/home/moriyama/Downloads/Boat1193.mp4"), batch_size=batch_size))
...
for epoch in range(epochs):
	for itr, (img1, img2, mask, flow) in enumerate(dataloader):
		...
		#
		if (itr+1)%10000 ==0 :
			torch.save(model.state_dict(), '%s/final_reconet_epoch_%d_itr_%d.pth' % ("output", epoch, itr//1000))


		print('[%d/%d][%d/%d] LOSS: %.4f SL: %.4f CL: %.4f FTL: %.4f OTL: %.4f RL: %.4f'
						% (epoch, epochs, itr, len(dataloader),
							loss, style_loss, content_loss , f_temporal_loss, o_temporal_loss, reg_loss))
	torch.save(model.state_dict(), '%s/reconet_epoch_%d.pth' % ("output", epoch))
